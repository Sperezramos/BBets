...
We are going to build a array with 40 different substitute teachers doing the same bet Brownlow did, and we will see what the average is that teachers made.

```{r}
subs40 <- vector("numeric")
for(i in 1:40){
  subs40[i]<-BrLw(numClass = 35, odds = 2, countoDo = 637)
}

favstats(subs40)
lcl <- mean(subs40) - 2*sd(subs40)
ucl <- mean(subs40) + 2*sd(subs40)
options(digits = 1)
print(paste(lcl,ucl))
```
